...
Node *topK_op_handler(Graph *graph, Node *node) {
    ...
    if (node->inputs.size() == 2) {
        ...
        // WARN(yiakwy) : fill_constant : convert `int64_t` to `int` while popart requires INT64 to create constant node
        // VarType_Type_INT64 = 3
        /*
        auto dtype = var->GetDataType();
        if (dtype != 3) {
            var->SetDataType(proto::VarType::INT64);
        }
         */
    ...
    for (auto* o_node : node->outputs) {
        ...
        VLOG(10) << "[topK_op_handler] output node(" << name << ") dtype : " << dtype;
        
    }
    VLOG(10) << "[topK_op_handler] leave the handler.";
    return CreateBaseOp(graph, node, "popart_topk", 
                        inputs, {node->outputs[1], node->outputs[0]}, attrs);
}
