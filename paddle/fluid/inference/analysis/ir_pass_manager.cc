...
void IRPassManager::CreatePasses(Argument *argument,
  ...
  for (const std::string &pass_name : passes) {
    ...
    if (pass_name == "ipu_graph_builder_pass" || pass_name == "ipu_runtime_replacer_pass") {
      // Need to be modified in future
      std::vector<std::string> feed_list = { "a", "b" };
      std::vector<std::string> fetch_list = { "tmp_0" };
      pass->Set("feed_list", new std::vector<std::string>(std::move(feed_list)));
      pass->Set("fetch_list", new std::vector<std::string>(std::move(fetch_list)));
    }
#endif
    pre_pass = pass_name;
