...
if enable_test:
    ...
    custom_ops = load(
        name="custom_popart_pattern_demo",
        sources=[
            f"{cur_dir}/custom_nop_op.cc",
            f"{cur_dir}/custom_popart_pattern.cc",
        ],
    ...
    def _test_base(self, run_ipu=True):
        ...
        with fluid.scope_guard(scope):
            with paddle.static.program_guard(main_prog, startup_prog):
                ...
                x = paddle.fluid.layers.elementwise_add(x, x)
                x = paddle.fluid.layers.relu(x)
                out = paddle.fluid.layers.relu(x, **self.attrs)
                fetch_list = [out.name]
